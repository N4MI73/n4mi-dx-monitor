...
- Lets you pin priority Needed entities so they stay near the top of the
  list even when nothing's currently active for them.
- Lets you drill into recent activity and spot history without leaving the
  device.
- Includes on-device Wi-Fi setup (a real captive portal) -- no need to
  hardcode credentials or reflash firmware just to change networks.
...
- **PropMon** (this series' own propagation instrument) -- current band
  condition for a spotted station's frequency, consumed as a read-only
  external API. Optional in the sense that DXMon degrades gracefully (no
  band-condition indicator shown) if PropMon isn't reachable.
...
├── images/           -- screenshots/photos used in this documentation.
